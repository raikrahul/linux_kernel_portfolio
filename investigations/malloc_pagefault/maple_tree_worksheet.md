...
3. WHY STEP 3: mmap() creates VMA struct in kernel RAM, this struct is what mas_walk() will find later, without VMA the tree is empty and page fault returns NULL → SOURCE: `include/linux/mm_types.h:649` defines `struct vm_area_struct` → KEY FIELDS: vm_start(offset 0, 8 bytes), vm_end(offset 8, 8 bytes), vm_mm(offset 16, pointer to mm_struct), vm_flags(offset ~40, permission bits), detached(offset ~148, bool checked by lock_vma_under_rcu:5724) → DRAW VMA struct after mmap: `+--vm_area_struct @ [SIMULATED]--+` `| vm_start = 0x78d7ce727000      |` `| vm_end   = 0x78d7ce728000      |` `| vm_flags = 0x73 [DERIVED]      |` `+--------------------------------+` → DERIVATION of vm_flags: PROT_READ|PROT_WRITE → VM_READ(0x1)+VM_WRITE(0x2)+VM_MAYREAD(0x10)+VM_MAYWRITE(0x20)+VM_MAYEXEC(0x40)=0x1+0x2+0x10+0x20+0x40=0x73 ✓ → VERIFY: 0x78d7ce728000 > 0x78d7ce727000 ✓.
